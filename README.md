...
| 필드 | 필수 | 설명 |
|---|:---:|---|
| `name` | △ | 기자 이름. **이름으로만 검색**합니다. **비우면 키워드만으로 검색** (이 경우 `include_keywords` 필수) |
| `press` | ❌ | 언론사명. **출처 도메인 필터**에 사용됩니다(아래 참고). 빈 문자열이면 도메인 제한 없음 |
| `domain` | ❌ | 기사 출처 도메인(문자열 또는 배열). 예: `"mtn.co.kr"`. 지정 시 `press` 추론보다 **우선** |
| `include_keywords` | △ | **모든** 키워드가 제목/요약에 있어야 함 (AND). `name` 이 비어 있으면 검색어로도 사용되며 **하나 이상 필수** |
| `exclude_keywords` | ❌ | **하나라도** 제목/요약에 있으면 제외 (NOT) |

> **검색·매칭 동작:** 네이버 검색은 **이름(+포함 키워드)** 으로만 합니다. 언론사는 검색어에 넣지 않고, 기사의 출처(`originallink`) **도메인이 일치할 때만** 통과시킵니다. 매체마다 바이라인 형식이 달라 `"이름 기자"` 식 정확구문은 최신 기사를 놓치고, 동명이인 문제도 있기 때문입니다. (`mtn.co.kr` 지정 시 `news.mtn.co.kr` 같은 서브도메인도 일치)
>
> `press` 로부터 도메인을 추론하는 기본 매핑이 내장돼 있습니다(예: `MTN`→`mtn.co.kr`, `전자신문`→`etnews.com`, `연합뉴스`→`yna.co.kr`). 매핑에 없는 언론사는 `domain` 필드로 직접 지정하세요. 도메인이 전혀 지정되지 않으면 출처 제한 없이 이름 검색 결과를 모두 받습니다.
>
