...
- **발행 시각 기준 최근 5분 이내 기사만 전송**합니다. cron 주기(5분)에 맞춰, 검색 결과 중 방금 올라온 기사만 골라 보냅니다. 따라서 이력이 비어 있어도 과거 기사가 한꺼번에 쏟아지지 않습니다.
  - cron 지연으로 누락이 걱정되면 `LOOKBACK_MINUTES` Secret(또는 환경변수)으로 윈도우를 늘릴 수 있습니다. 예: `7`. 미설정 시 기본 5분.
  - URL 중복 제거가 함께 동작하므로 윈도우를 늘려도 같은 기사가 두 번 전송되지는 않습니다.
