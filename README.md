...
| `TELEGRAM_CHANNELS` | ❌ | **특정 기자만 다른 봇/채팅방으로도 추가 전송**할 때 쓰는 이름표 채널 목록 JSON(아래 5.4 참고) |
...
| `extra_channels` | ❌ | 이 기자의 알림을 **기본 채널에 더해 추가로** 보낼 이름표 채널(`TELEGRAM_CHANNELS` 의 키). 문자열 또는 배열(`"seo"` / `["seo","desk"]`). 아래 5.4 |
...
### 5.4 특정 기자만 다른 텔레그램 봇(채팅방)으로도 추가 전송하기

기존 알림은 그대로 두고, 특정 기자의 기사만 **다른 봇 / 다른 채팅방으로도 한 번 더** 보내고 싶을 때(예: 서정근 기자 기사는 기본 채널 + 별도 봇), Secret 하나만 추가하면 됩니다.

**`TELEGRAM_CHANNELS` Secret 등록** — 이름표(키)별 봇 토큰·chat_id·대상 기자를 담은 JSON 객체(한 줄):

```json
{"seo":{"bot_token":"7123456789:AAH...","chat_id":"-1009876543210","reporters":["서정근"]}}
```

- 이름표(`seo`)는 자유롭게 정합니다. 여러 개를 정의해도 됩니다: `{"seo":{...},"desk":{...}}`
- `reporters` 에는 REPORTERS 의 기자 `name`(또는 이름이 없는 키워드 항목이면 표시 라벨)을 적습니다. 문자열 하나 또는 배열.
- `chat_id` 는 기본 채널과 똑같이 **쉼표로 여러 대상** 지정 가능(`"-100111,-100222"`).
- 봇 만들기·초대·chat_id 확인 절차는 Step 3~5 와 같습니다(새 봇도 대상 채팅방에 **초대/관리자 등록**이 필요).
- (선택) 같은 항목에 `"line_token"`, `"line_group_id"` 를 넣으면 그 이름표 채널이 라인으로도 함께 전송됩니다.

**대안: `REPORTERS` 쪽에서 지정** — 채널의 `reporters` 대신(또는 함께) 기자 항목에 `"extra_channels"` 를 적어도 됩니다. 두 방식은 합쳐져 적용됩니다.

```json
[
  {"name":"김주환","press":"MTN"},
  {"name":"서정근","naver_journalist":"030/38807","extra_channels":"seo"}
]
```

동작:

- 연결된 기자(서정근)의 기사는 **기본 채널(`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`)로 지금처럼 나가고, `seo` 채널로도 추가로** 전송됩니다.
- 연결되지 않은 기자(김주환)는 기본 채널로만 갑니다(변화 없음).
- 여러 채널에 더 보내려면 배열로 적습니다: `"extra_channels":["seo","desk"]`
- `reporters` 의 이름이 REPORTERS 에 없거나, `extra_channels` 값이 `TELEGRAM_CHANNELS` 에 없거나, 토큰/chat_id 가 비어 있으면 **실행 시작 시 오류로 즉시 종료**합니다(오타로 조용히 누락되지 않음). Actions 로그의 `[ERROR]` 줄을 확인하세요.
- 각 기사는 채널마다 한 번씩만 전송되며(URL 기준 중복 방지), `data/sent_articles.json` 이력에 추가 채널 목록(`extra_channels`)이 함께 기록됩니다.
- **Notify self-test** 워크플로우를 실행하면 기본 채널과 모든 이름표 채널에 테스트 메시지를 보내고, 기자별로 어느 채널에 연결됐는지 로그로 보여줍니다.
